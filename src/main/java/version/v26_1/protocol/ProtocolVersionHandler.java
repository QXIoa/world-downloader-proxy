...
public class ProtocolVersionHandler {
    ...
    public static ProtocolVersionHandler getInstance() {
        ...
        var resourceStream = ProtocolVersionHandler.class.getClassLoader().getResourceAsStream(VERSION_PATH);
        if (resourceStream == null) {
            throw new RuntimeException("Missing protocol versions resource: " + VERSION_PATH);
        }
        Reader file = new InputStreamReader(resourceStream);
