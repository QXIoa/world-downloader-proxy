 ...
 * not stored.
 */
public class DimensionRegistry implements IDimensionRegistry {
    ...
    @Override
    public String getDimensionTypeName(String dimensionType) {
        DimensionType type = getDimensionType(dimensionType);
        return type == null ? null : type.getName();
    }

    @Override
    public int[] getDimensionTypeBounds(String dimensionType) {
        DimensionType type = getDimensionType(dimensionType);
        if (type == null || type.getProperties() == null) { return null; }
        se.llbit.nbt.CompoundTag props = type.getProperties();
        se.llbit.nbt.Tag minYTag = props.get("min_y");
        se.llbit.nbt.Tag heightTag = props.get("height");
        if (minYTag == null || heightTag == null) { return null; }
        return new int[] { ((se.llbit.nbt.IntTag) minYTag).getData(), ((se.llbit.nbt.IntTag) heightTag).getData() };
    }
