...
public class RegionImage {
    ...
    private static final int SIZE = WorldGeometry.SECTION_WIDTH * WorldGeometry.REGION_SIZE;;
    ...
    private RegionImage(WritableImage image, Path path, Coordinate2D coords) {
        ...
        chunkOverlay = new WritableImage(WorldGeometry.REGION_SIZE, WorldGeometry.REGION_SIZE);
    ...
    private void fillOverlay(Color c) {
        for (int i = 0; i < WorldGeometry.REGION_SIZE; i++) {
            for (int j = 0; j < WorldGeometry.REGION_SIZE; j++) {
    ...
    private void drawChunkToImage(Coordinate2D local, Image chunkImage) {
        int size = WorldGeometry.SECTION_WIDTH;
