...
import core.gui.markers.MapMarkerHandler;
import core.gui.markers.PlayerMarker;
...
public class GuiMap {
    ...
    private Collection<IPlayerEntity> otherPlayers;
    ...
    void initialize() {
        ...
        IWorldManager manager = Config.getVersionModule().getWorldManager();
    ...
    void setChunkLoaded(CoordinateDim2D coord, IChunk chunk) {
        ...
        IChunkImageFactory imageFactory = chunk.getChunkImageFactory();
    ...
    private void drawDebugHighlight(GraphicsContext graphics) {
        ...
        if (blocksPerPixel > 1) {
            ...
            graphics.setFill(Color.rgb(0, 0, 0, .2));
            graphics.fillRect(drawX, drawY, gridSize / 32.0, gridSize / 32.0);
        }
    }
    ...
    private void drawEntities() {
        ...
        if (Config.renderOtherPlayers()) {
            for (IPlayerEntity player : otherPlayers) {
     ...
     */
    private void drawOtherPlayer(GraphicsContext graphics, IPlayerEntity player) {
    ...
    public void setStatusMessage(String str) {
        if (str.equals("") && Config.getVersionModule().getWorldManager().isPaused()) {
