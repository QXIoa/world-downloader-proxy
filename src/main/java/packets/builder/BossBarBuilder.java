...
/**
 * Builds {@code BossEvent} (boss bar) packets. Knows about the version-specific
 * serialisation of the title field (NBT for 1.20.6+, JSON string for earlier versions)
 * but does not know anything about what the boss bar is used for.
 */
public final class BossBarBuilder {
    ...
    private static void writeTitle(PacketBuilder pb, String title) {
        ...
        if (Config.versionReporter().packetFormat().chatIsNbt()) {
            pb.writeNbtDirect(chat.toNbt());
        } else {
            pb.writeString(chat.toJson());
        }
    }
}
